...
import 'package:myapp9/widgets/city_picker/modal/base_citys.dart';
...
class _BaseView extends State<BaseView> {
  ...
  void _initController() {
    ...
    if (showType.contain(ShowType.p)) {
      provinceController = new FixedExtentScrollController(initialItem: provinces.indexWhere((Point p) => p.code == targetProvince.code));
    }
    if (showType.contain(ShowType.c)) {
      cityController = new FixedExtentScrollController(initialItem: targetProvince.child.indexWhere((Point p) => p.code == targetCity!.code));
    }
    if (showType.contain(ShowType.a)) {
      areaController = new FixedExtentScrollController(initialItem: targetCity!.child.indexWhere((Point p) => p.code == targetArea!.code));
    }
    // 增加第4级(村/镇)选择
    if (showType.contain(ShowType.v)) {
      villageController = new FixedExtentScrollController(initialItem: targetArea!.child.indexWhere((Point p) => p.code == targetVillage!.code));
    }
  }
  ...
  void _initLocation(String? locationCode) {
    ...
    if (locationCode != null) {
      ...
      targetProvince = cityTree.initTreeByCode(_locationCode) as Point;

      /// 为用户给出的locationCode不正确做一个容错
      if (targetProvince.isNull) {
        targetProvince = cityTree.initTreeByCode(provinces.first.code!) as Point;
      }
      ...
      targetProvince = cityTree.initTreeByCode(int.parse(widget.provincesData.keys.first)) as Point;
    }
    // 尝试试图匹配到下一个级别的第一个,
    if (targetCity == null) {
...
class _MyCityPicker extends StatefulWidget {
  ...
  _MyCityPicker({this.key, this.controller, this.isShow = false, required this.changed, this.itemList, this.itemExtent, this.itemBuilder, this.value});
