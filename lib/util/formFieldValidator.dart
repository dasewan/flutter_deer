...
class RequiredValidator extends TextFieldValidator {
  ...
  @override
  String? call(String? value) {
    isValid(value) ? null : Toast.show(errorText, duration: 3000);
    return isValid(value) ? null : errorText;
  }
}
...
class NumberValidator extends TextFieldValidator {
  ...
  final int max;
  /// regex pattern to validate email inputs.
  // final String _emailPattern = "^\\d{$min,$max}\$";

  NumberValidator({required this.min, required this.max,required String errorText}) : super(errorText);

  @override
  bool isValid(String? value) => hasMatch("^\\d{$min,$max}\$", value!, caseSensitive: false);
...
class StringValidator extends TextFieldValidator {
  ...
  StringValidator({required this.min, required this.max,required String errorText}) : super(errorText);

  @override
  bool isValid(String? value) => hasMatch("^[a-z]{$min,$max}\$", value!, caseSensitive: false);
