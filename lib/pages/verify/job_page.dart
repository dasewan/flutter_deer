...
import 'package:myapp9/pages/verify/presenters/job_page_presenter.dart';
...
import 'iviews/job_i_mvp_view.dart';

/// design/6/index.html#artboard2
class JobPage extends StatefulWidget {
  const JobPage({Key? key, this.id, this.name}) : super(key: key);
  final int? id;
  final String? name;
...
class _JobPageState extends State<JobPage> with BasePageMixin<JobPage, JobPagePresenter>, SingleTickerProviderStateMixin implements JobIMvpView {
  late JobPagePresenter _contactPagePresenter;
  final List<QVerifyItemEntity>? _verifyItem = SpUtil.getObj<QVerifyItemEntity>(Constant.jobVerifyItem, (v) => $QVerifyItemEntityFromJson(v))!.children;
  ...
  @override
  JobPagePresenter createPresenter() {
    _contactPagePresenter = JobPagePresenter();
        ...
        body: SafeArea(
          child: SliverDemoPage(
            ...
            child: MyScrollView(
                    ...
                    MyButton(
                      onPressed: () {
                        if ((_formKey.currentState as FormState).validate()) {
                          ...
                          //验证通过提交数据
                          _contactPagePresenter.store(_submittedField, true);
              ...
              children: <Widget>[
                // const SizedBox(height: 80, child: ProcessTimelinePage(processIndex: 2,),),
                Form(
                    key: _formKey,
