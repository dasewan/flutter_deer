...
class LoanBankPage extends StatefulWidget {
  const LoanBankPage({Key? key, this.id, this.name}) : super(key: key);
  final int? id;
  final String? name;

  @override
  _LoanBankPageState createState() => _LoanBankPageState();
}

class _LoanBankPageState extends State<LoanBankPage>
    with BasePageMixin<LoanBankPage, LoanBankPagePresenter>, SingleTickerProviderStateMixin
    implements LoanBankIMvpView {
  late LoanBankPagePresenter _contactPagePresenter;
  final List<QVerifyItemEntity>? _verifyItem = SpUtil.getObj<QVerifyItemEntity>(Constant.loanBankVerifyItem, (v) => $QVerifyItemEntityFromJson(v))!.children;
  ...
  @override
  LoanBankPagePresenter createPresenter() {
    _contactPagePresenter = LoanBankPagePresenter();
                ...
                children: [
                  ...
                  CupertinoButton(
                    ...
                    onPressed: () {
                      ...
                      return Navigator.of(ctx as BuildContext).pop();
                    },
                  )
                ],
              ),
